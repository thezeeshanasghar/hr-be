...
const InsertCompany = async (req, res) => {
	...
	try {                                      
		var query = `insert into [dbo].[Company] (Code, CompanyName, Address, Contact, Email, CountryCode, StartDate, RegistrationNo, TaxationNo, SocialSecurityNo, EOBINo,PayrollFormula,CompanyDeduction,EmployeeDeduction) values('`+req.body.Code+`','`+req.body.CompanyName+`','`+req.body.Address+`','`+req.body.Contact+`','`+req.body.Email+`','`+req.body.CountryCode+`',getdate(),'`+req.body.RegistrationNo+`','`+req.body.TaxationNo+`','`+req.body.SocialSecurityNo+`','`+req.body.EOBINo+`','`+req.body.payrollformula+`','`+req.body.companyPercentage+`','`+req.body.employeePercentage+`' );
					...
					(BankId,CompanyId,CurrencyId,AccNo)
					SELECT 
					 [BankId],@Id ,[CurrencyId], [AccNo]
					FROM OPENJSON('`+req.body.Accounts+`') WITH 
					(
					[BankId] VARCHAR(500), 
					[CurrencyId] VARCHAR(500),
					[AccNo] VARCHAR(500)
					)`;
					console.log(query);
		const pool = await poolPromise
...
const UpdateCompany = async (req, res) => {
	try {
		...
		delete from [dbo].[CompanyBankAccounts] where CompanyId='`+req.params.Id+`'
		insert into [dbo].[CompanyBankAccounts]
		(BankId,CompanyId,CurrencyId,AccNo)
		SELECT 
		 [BankId],'`+req.params.Id+`' ,[CurrencyId], [AccNo]
		FROM OPENJSON('`+req.body.Accounts+`') WITH 
		(
		[BankId] VARCHAR(500), 
		[CurrencyId] VARCHAR(500),
...
const GetCompanyBank = async (req, res) => {
	try {
		var query = `
		select 
AccNo,Acc.Id
from 
[dbo].[CompanyBankAccounts] Acc Inner Join 
[dbo].[Bank] banks ON banks.Id=Acc.BankId
Where Acc.CompanyId ='`+req.params.Id+`'
		`;
		const pool = await poolPromise
		const result = await pool.request()
			.query(query, function (err, profileset) {
...
module.exports = { GetAllCompanies,GetCompanyById,InsertCompany,UpdateCompany,DeleteCompany,GetSelectiveComponies,GetCompanyBank};